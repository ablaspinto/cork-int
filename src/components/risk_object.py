...
def __init__(self,cve_id: str,description: str = "",cvss_score: float = 0.0,epss_score: float = 0.0,
             ...
             kev_status: bool = False,severity_level):
...
def repr_severity_level():
    if self.__severity_level >= 9.0:
        return COLOR_RATING.get("red") +"CRITICAL" + COLOR_RATING.get("red")
    if self.__severity_level >= 7.0:
        return COLOR_RATING.get("red") +"ACCELERATED" + COLOR_RATING.get("red")
    if self.__severity_level >= 4.0:
        return COLOR_RATING.get("yel") +"ROUTINE" + COLOR_RATING.get("yel")
    if self.__severity_level >= 4.0:
        return COLOR_RATING.get("cyan") +"ROUTINE" + COLOR_RATING.get("cyan")
...
def assess_risk(self) -> float:
    ...
    self.__severity_level = kev_score + cvss_score + epss_score
...
def __str__(self) -> str:
    return 
        ...
        f"Risk Assesment: {self.repr_severity_level()}" +
        f"Risk Score": {self.__severity_level}
